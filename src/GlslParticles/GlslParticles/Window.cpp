...
int Window::Initialise()
{
	...
	// Create the window
	mainWindow = glfwCreateWindow(width, height, "Loading...", NULL, NULL);
	if (!mainWindow)
	{
...
void Window::update()
{
	double dt = glfwGetTime() - lastFPStime;
	lastFPStime = glfwGetTime();

	glfwSetWindowTitle(mainWindow, (std::to_string(1.0 / dt) + " fps").c_str());
}
